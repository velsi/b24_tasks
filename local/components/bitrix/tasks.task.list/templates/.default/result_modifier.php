...
if (!function_exists('prepareTaskRowTitle')) {
    function prepareTaskRowTitle($row, $arParams)
    {
        ...
        $cssClass = 'task-status-text-color-' . \tasksStatus2String(($row['REAL_STATUS'] == \CTasks::STATE_EVALUATION) ? $row['REAL_STATUS'] : $row['STATUS']);
        $title = '<a href="' .
            $taskUrl .
            '" class="task-title ' .
            $cssClass .
...
if (!function_exists('prepareTaskGroupActions')) {
    function prepareTaskGroupActions($arResult, $arParams)
    {
        ...
        $actionList[] = array(
            'NAME' => GetMessage('TASKS_LIST_GROUP_ACTION_SET_PARENT'),
            'VALUE' => 'setparent',
            'ONCHANGE' => array(
                array(
                    ...
                    'DATA' => array(
                        array(
                            ...
                            'ID' => 'action_set_patent',
                            'NAME' => 'parent',
                            'ITEMS' => getTasksToParent($arResult)                        )
                    )
                )
            )
        );  // setparent
...
if(!function_exists('getTasksToParent')){
    function getTasksToParent($arResult): array
    {
        $result = [];
